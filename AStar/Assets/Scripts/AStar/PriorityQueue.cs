...
namespace PriorityQueue
{
    ...
    {
        public static void Enqueue<T>(this List<T> _pq, T item) where T : IComparable<T>
        {
            _pq.Add(item);
            BubbleUp(_pq);
        }

        public static T Dequeue<T>(this List<T> _pq) where T : IComparable<T>
        {
        ...
        private static void BubbleUp<T>(List<T> _pq) where T : IComparable<T>
        {
        ...
        private static void Swap<T>(List<T> _pq,int index1,int index2) where T : IComparable<T>
        {
            var tmp = _pq[index1];
        ...
        private static void MoveLastItemToTheTop<T>(List<T> _pq) where T : IComparable<T>
        {
        ...
        private static void SinkDown<T>(List<T> _pq) where T : IComparable<T>
        {
            var lastIndex = _pq.Count - 1;
            var parentIndex = 0;
